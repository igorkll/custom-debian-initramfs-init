...
* startupsound_afterModulesLoading - it tries to play the sound after loading the kernel modules
...
* startupsound_afterLogoShowOnlyAfterModules - It plays the sound as soon as the logo is displayed. it only works if earlysplash is specified. even if the logo was shown before loading the modules (this is possible on some platforms, but not on all) then the sound will still be played after loading the modules.
